...
public class DepartmentCaseUpdatedHandler(
    EmergencyDbContext db,
    IProducer<string, string> producer,
    PollRegistry pollRegistry,
    IRedisCache cache) : IDepartmentCaseUpdatedHandler
{
        ...
        if (statusStr.Equals("CLOSED", StringComparison.OrdinalIgnoreCase))
        {
            ...
            for (var attempt = 0; attempt < 3 && !resolved; attempt++)
            {
                ...
                try
                {
                    await db.SaveChangesAsync(ct);
                    resolved = true;
                    await cache.InvalidateAsync(EmergencyMapper.CacheKey(fresh.CityId), ct);
