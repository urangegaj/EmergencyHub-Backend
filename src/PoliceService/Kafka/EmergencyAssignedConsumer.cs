...
{
    private const string DepartmentFilter = "Police";
    ...
    private async Task HandleAsync(string json, CancellationToken ct)
    {
        ...
        {
            logger.LogDebug(
                "Police case already exists for emergency {EmergencyId}, skipping",
                emergencyId);
            return;
        }

        var now = DateTime.UtcNow;
        db.Cases.Add(new Models.PoliceCase
        {
            Id = Guid.NewGuid(),
        ...
        logger.LogInformation(
            "Created police case for emergency {EmergencyId} in city {CityId}",
            emergencyId,
            cityId);
    }
}
