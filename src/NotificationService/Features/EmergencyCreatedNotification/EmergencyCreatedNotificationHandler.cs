...
public class EmergencyCreatedNotificationHandler(
    NotificationDbContext db,
    ILogger<EmergencyCreatedNotificationHandler> logger) : IEmergencyCreatedNotificationHandler
{
    public async Task HandleMessageAsync(string? json, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(json))
        {
            logger.LogWarning("Received null or empty emergency.created message, skipping");
            return;
        }
