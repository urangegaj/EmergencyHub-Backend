...
{
    private const string DepartmentFilter = "Fire";
    ...
    private async Task HandleAsync(string? json, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Received null or empty message on {Topic}", Topics.EmergencyAssigned);
            return;
        }
        ...
        {
            logger.LogDebug(
                "Fire case already exists for emergency {EmergencyId}, skipping",
                emergencyId);
            return;
        }

        var now = DateTime.UtcNow;
        db.Cases.Add(new Models.FireCase
        {
            Id = Guid.NewGuid(),
            EmergencyId = emergencyId,
            CityId = cityId,
            Status = DomainFireCaseStatus.OPEN,
            CreatedAt = now,
            UpdatedAt = now
        });
