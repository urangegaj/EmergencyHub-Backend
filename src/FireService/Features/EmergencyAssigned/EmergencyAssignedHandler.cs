    ...
    public async Task HandleAsync(string? json, CancellationToken ct)
    {
        ...
        if (exists)
        {
            logger.LogInformation(
                "Fire case already exists for emergency {EmergencyId}, skipping (idempotent)", emergencyId);
            return;
        }

        var now = DateTime.UtcNow;
        db.Cases.Add(new FireService.Models.FireCase
        {
            Id = Guid.NewGuid(),
        ...
        logger.LogInformation(
            "Created fire case for emergency {EmergencyId} in city {CityId}", emergencyId, cityId);
    }
}
