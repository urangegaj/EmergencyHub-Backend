...
public class GetCasesHandler(FireDbContext db) : IGetCasesHandler
{
    public async Task<GetCasesResponse> HandleAsync(GetCasesRequest request, ServerCallContext context)
    {
        ...
        response.Cases.AddRange(cases.Select(c =>
            FireMapper.MapCase(c, c.AssignedUnitId.HasValue ? units.GetValueOrDefault(c.AssignedUnitId.Value) : null)));
