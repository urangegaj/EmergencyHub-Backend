...
public static class FireRoutes
{
    public static void MapFireRoutes(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(status) &&
                    ProtoEnumParse.TryParse<FireCaseStatus>(status, out var parsed))
                    req.Status = parsed;
            ...
            CancellationToken ct) =>
        {
            ...
            try
            {
                if (!ProtoEnumParse.TryParse<FireCaseStatus>(body.Status, out var status))
            ...
            CancellationToken ct) =>
        {
            ...
            try
            {
                if (!ProtoEnumParse.TryParse<FireUnitStatus>(body.Status, out var status))
                    return Results.BadRequest($"Unknown status: {body.Status}");
